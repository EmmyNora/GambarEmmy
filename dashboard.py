...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        # ==========================
            ...
            with st.spinner(f"🧠 Mengecek {uploaded_file.name}..."):
                # Jalankan YOLO dulu buat cek apakah ini gambar karakter
                yolo_results = yolo_model.predict(img, conf=0.6, verbose=False)
                boxes = yolo_results[0].boxes

                # Kalau YOLO mendeteksi objek (Spongebob/Patrick) → lewati klasifikasi
                if boxes is not None and len(boxes) > 0:
                    st.warning("💖 Gambar ini terdeteksi sebagai karakter (Spongebob/Patrick).")
                    st.info("🚫 Klasifikasi Indoor/Outdoor dilewati agar hasil tidak salah interpretasi.")
                    st.image(yolo_results[0].plot(), caption="🎀 Hasil Deteksi Objek 🎀", use_container_width=True)

                # Kalau tidak ada objek → baru klasifikasi Indoor/Outdoor
                else:
